...
# %%
pred_valid = pd.DataFrame(model.predict(x_valid).argmax(axis=1), columns=["health"], index=valid_index)
print(f1_score(y_valid, pred_valid, average="macro"))
# %%
print(pred_valid.value_counts()/len(pred_valid))

# %%
submit_name = "optuna_lgbm"
predict = pd.DataFrame(model.predict(x_test).argmax(axis=1), columns=["health"], index=test_index)
